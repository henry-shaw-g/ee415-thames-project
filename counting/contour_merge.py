...
def _get_bbox_dimension_cells(cells_dim, cell_size, contour):
    ...
    cell_j0 = max(0, math.floor(by / cell_size))
    cell_j1 = min(cells_dim[0] - 1, math.floor((by + bh) / cell_size))
    cell_i0 = max(0, math.floor(bx / cell_size))
    cell_i1 = min(cells_dim[1] - 1, math.floor((bx + bw) / cell_size))
...
def _populate_grid(grid, contours):
    for contour in contours:
        cells_y, cells_x = _get_bbox_dimension_cells((len(grid), len(grid[0])), GRID_SIZE, contour)
        for i in cells_y:
            for j in cells_x:
                grid[i][j].append(contour)

class Merger:
    def __init__(self, image, contours, settings):
                ...
                for j in cells_x:
                    for other in grid[i][j]:
                        ...
                        if other.get_type() is Contour.type.single_bee:
                            if self._check_single_mergeable(other, contour):
    ...
    def _check_single_mergeable(self, bigger, smaller):
        ...
        (intersection, _) = cv.intersectConvexConvex(bigger.contour, smaller.contour)
        IOU = intersection / (bigger.area + smaller.area - intersection[0])
        return IOU < self.settings["contour_merge_IOU_threshold"]
